...
function disable_folder_button(){
  ...
  let cd = document.getElementById('cur_img_dir').value
...
function mass_action(action) {
  ...
  for (let i=0;i<selected.length;i++){
    if(action == 'move'){
      ...
      formData.append('cur_img_dir', document.getElementById('cur_img_dir').value);
  ...
  select_folder(document.getElementById('cur_img_dir').value);
  close_model();
}
...
function select_folder(folder){
  ...
  xhr.addEventListener("readystatechange", function() {
    if(this.readyState === 4) {
      ...
      document.getElementById('cur_img_dir').value = obj['path'];
      document.getElementById('img_res').innerHTML = '';
...
function upload_images(){
  ...
  xhr.open("POST", "/upload/" + document.getElementById('cur_img_dir').value);
  xhr.send(formData);
...
function upload_images_form(){
    let d = document.createElement("div")
    let p = document.createElement("p")
    let text = 'Please select images to upload to the ' + document.getElementById('cur_img_dir').value + ' folder.'
    p.innerHTML = text;
    d.appendChild(p);
    let form = document.createElement("form");
